...
function sliderFn() {
     ...
     */
    var i = 0;
    var preloadImg = function (url, success, error) {
        var img = new Image();
        img.onload = function () {
            success && success();
    ...
    var Slider = function(container,options){
        ...
        $.extend(this, {
            ...
            activeTriggerCls: 'selected',  //触发元素内子元素的激活样式
    ...
    $.extend(Slider.prototype,{
        ...
        init : function(){
            ...
            if(this.loop){  //复制首尾以便循环
                var firstp = panels[0].cloneNode(true);
            ...
            obj.find(cls).each(function(n,item){
                ...
                if(src){
                    preloadImg(src, function (){
                        el.attr('src',src).removeAttr('data-img').removeClass(lazy);
                    }, function () {
                        el.removeClass(lazy);
                    });
                }
        ...
        attachTo : function(obj, options) {
            ...
            return obj.each(function(n,item){
                if (!item.getAttribute('l')) {
                    ...
                    Slider.cache.push(new Slider(item, options));
                }
            });
        }
    });
...
var SliderArea = React.createClass({
	...
	render: function() {
		...
		var listLi = list.map(function(data) {
			if(data && data.jumpLink) {
				return (
					<li>
						<a className="linka" href={data.jumpLink}>
							<img src={data.imgUrl} />
						</a>
					</li>
				)
		...
		return (
			<div className="SliderArea" id={moduleId} style={style} onTouchmove={function(e){self.touchmove(e);console.log(333);}}>
				<div className={getRightClassName} id={moduleId+'maodian'}></div>
				<div className="slider-big" style={{height:heightRem}}>
					<div className="slider-big-outer">
						<ul className="slider-big-wrap" onTouchmove={function(e){self.touchmove(e);console.log(333);}} >
							{listLi}
						</ul>
					</div>
					<div className="slider-status clear"></div>
				</div>
			</div>
		);
	}
