...
var RedEnvelope = React.createClass({
	componentDidMount:function() {
	    var imgUrl = config && config.imgUrl;		
	    var imgUrlDataArr = imgUrl && imgUrl.split(',') || [];
	    if(imgUrlDataArr && imgUrlDataArr.length==1) {
        	var phone = document.querySelector('.phoneForOnePage');
        	var top = phone.getBoundingClientRect().top;
        	var imgRoll = document.querySelector('.imagebg img');
	        var htmlFontSize = parseInt(document.getElementsByTagName('html')[0].style.fontSize);
	        var limit = dpr*420;
	        var h = 11.2*htmlFontSize;
	        if(top < limit && top < h) {
	        	imgRoll.style.height = top + 'px';
	        }
	    }

        var IOSDownload = lib.getByUrl('IOSDownload',this.props.imgUrl) || "https://itunes.apple.com/app/apple-store/id918137885?pt=96492967&ct=zirushenghuojie&mt=8" ;
	...
	render: function() {
		...
		var isColorLight = config && config.isColorLight;
		...
		if(isColorLight == 1) {
			...
			var colorJson = {
				...
				topColorBefore:'#808080',
				topBgColorBefore:'#ffffff',
		...
		//领取前边框的背景颜色
		var topBgColorBefore = colorJson.topBgColorBefore || "#000";
		//领取前立即领取的边框颜色
		...
		styleTopBefore.backgroundColor = topBgColorBefore;
		...
		var imgUrlDataArr = imgUrl && imgUrl.split(',') || [];
		if(imgUrlDataArr && imgUrlDataArr.length > 1) {
			var classWrap = "AllComponents";
			var classPhone = 'phone';
			var imgRoll = imgUrlDataArr.map(function(data) {
				return (
					<Img src={data} rname="imgRollAllList" size="640x640" />
				)
			});
		} else {
			var classWrap = "AllComponentsForOnePage";
			var classPhone = 'phoneForOnePage';
			var imgRoll = 	<div className='imgroll'>
		...
		return (
			<div className = {classWrap} style={bgColor} id={moduleId}>
				{imgRoll}
